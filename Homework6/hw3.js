...
for (i = 0; i < users.length; i++) {
    if (users[i].balance > 2000) {
        telephones.push(users[i].phone)
        
    }
}

document.write(`<p> Array of telephones of users whos balance is bigger than $2000: <br>${telephones.join("<br>")}</p>`)

let sumOfBalances = 0;

for (i = 0; i < users.length; i++) {
    sumOfBalances = sumOfBalances +users[i].balance
}

document.write(`<p>Sum of all users balances = ${sumOfBalances}</p>`)